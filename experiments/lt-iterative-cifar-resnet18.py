...
for epoch in range(N_EPOCH):
    ...
    train_losses, val_losses, train_accs, val_accs = net_base.train_epoch(epoch, optimizer, plot=False, data=plt_data, LOG=LOG)
...
for prune_epoch in range(pruning_iter):
    ...
    for ft_epoch in range(n_ft_epochs):
        train_losses, val_losses, train_accs, val_accs = net_ft.train_epoch(ft_epoch, optimizer, plot=False, data=plt_data, LOG=LOG, 
...
for epoch in range(N_EPOCH):
    ...
    plt_data = (train_losses, val_losses, train_accs, val_accs)
    train_losses, val_losses, train_accs, val_accs, stopped = net_retrain.train_epoch(epoch, optimizer, plot=False, data=plt_data, pruner=pruner_retrain, LOG=LOG)
    if stopped:
        break

torch.save(net_retrain.state_dict(), './checkpoints/iterative-pruning-cifar10/{}-reinit-trained'.format(EXPERIMENT_NAME))
save_data = {'train_losses': train_losses, 
             'val_losses': val_losses, 
             'train_accs': train_accs, 
             'val_accs': val_accs}
pd.DataFrame(save_data).to_csv('./experiment_data/iterative-pruning-cifar10/{}-reinit.csv'.format(EXPERIMENT_NAME), index=None)
...
#     plt_data = (train_losses, val_losses, train_accs, val_accs)
#     train_losses, val_losses, train_accs, val_accs = net_random.train_epoch(epoch, optimizer, plot=False, data=plt_data, pruner=pruner_reinit, LOG=LOG)
